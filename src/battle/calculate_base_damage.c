...
/* Type swap */
    ...
    if (attackerHoldEffect == HOLD_EFFECT_GRISEOUS_ORB && attacker->species == SPECIES_GIRATINA)
        gBattleMovePower = (120 * gBattleMovePower) / 100;
    if (attackerHoldEffect == HOLD_EFFECT_GRISEOUS_ORB && attacker->species == SPECIES_GIRATINAO)
        gBattleMovePower = (120 * gBattleMovePower) / 100;
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_EXPLOSION)
        defense /= 2;
    if (attacker->ability == ABILITY_IRON_FIST && (gCurrentMove == MOVE_BULLET_PUNCH || gCurrentMove == MOVE_COMET_PUNCH || gCurrentMove == MOVE_DIZZY_PUNCH || gCurrentMove == MOVE_DRAIN_PUNCH || gCurrentMove == MOVE_DYNAMIC_PUNCH || gCurrentMove == MOVE_FIRE_PUNCH || gCurrentMove == MOVE_FOCUS_PUNCH || gCurrentMove == MOVE_ICE_PUNCH || gCurrentMove == MOVE_MACH_PUNCH || gCurrentMove == MOVE_MEGA_PUNCH || gCurrentMove == MOVE_METEOR_MASH || gCurrentMove == MOVE_SHADOW_PUNCH || gCurrentMove == MOVE_SKY_UPPERCUT || gCurrentMove == MOVE_THUNDER_PUNCH))
        gBattleMovePower = (120 * gBattleMovePower) / 100;
